...
export default function Post({_id,title,text,tags,image,createdAt}) {
    return (
        <article className="grid place-items-center pb-12 max-w-4xl mx-auto mb-5">
              <Link to={`/posts/${_id}`} className=" w-full h-80"><img className="h-full object-cover w-full border-2 dark:border-white" src={image} alt={title} /></Link>
