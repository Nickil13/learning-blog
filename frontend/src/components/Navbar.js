...
export default function Navbar({isSmallScreenWidth, isSmallScreenHeight}) {
    ...
    const[isSideCategoryMenuShowing,setIsSideCategoryMenuShowing] = useState(false);
    ...
    const [recentIcons, setRecentIcons] = useState(iconData.slice(0,3).map((icon)=>icon.name) || []);
    ...
    <header className="transparent md:max-w-5xl mx-auto">
        {!isSmallScreenWidth && <div className="absolute font-display font-bold text-gray-700 dark:text-purple-300 p-5"><Link to="/">Learning Blog</Link></div>}
        
        {/* Mobile/Small screen Navbar */}
        {isSmallScreenWidth ? <nav className="fixed flex items-center py-4 px-6 bg-black bg-opacity-60 z-10 w-full top-0 left-0">
          ...
          <div className="flex items-center justify-end w-full">
            ...
            <div className="relative mx-2">
              ...
              {/* Category Menu Modal */}
              <NavDropdown isShowing={isCategoryMenuShowing} itemList={iconData} filteredItems={["home"]} onClick={handleCategoryClick} id="category-menu" location={"bottom"}/>
            </div>
            {/* User dropdown select for small screens */}
            <div className="relative mx-2">
              <AiOutlineUser className="nav-icon mx-2" onClick={()=>handleUserClick("nav")}/>
              <ul className={`absolute bg-black z-20 opacity-80 text-white p-4 rounded-md mt-5 top-full -left-1/4 ${!isUserNavShowing && 'hidden'}`} id="user-nav-menu">
                ...
                <li className="hover:text-purple-300 py-1 px-2 cursor-pointer" onClick={handleLogoutClick}>Logout</li>
              </ul>
            </div>
        ...
        <nav className={`fixed grid grid-cols-2 right-0 h-full place-items-center justify-center z-10 pointer-events-none ${!isSidebarOpen && "transform translate-x-1/2"} transition duration-500 ease-in-out`}>
          {/* Arrow Slide Bar */}
          ...
          {/* Nav Icons */}
          <div className={`relative grid bg-black bg-opacity-50 pointer-events-auto h-full pt-44`} ref={navContainer}>
            <DarkModeToggle/>
            {!isSmallScreenHeight ? 
            ...
            <ul className="flex flex-col grid-start-2 p-5 pointer-events-auto">
                ...
                return(
                  <li key={index} className='nav-icon mb-4' onClick={()=>{
                    handleNavClick(icon.name)
                  }}>{icon.icon}</li>
                )
              })}
              <li className='nav-icon mb-4'>
                <IoEllipsisHorizontal onClick={()=>setIsSideCategoryMenuShowing(true)}/>
                <NavDropdown isShowing={isSideCategoryMenuShowing} itemList={iconData} filteredItems={["home"]} onClick={handleCategoryClick} id="side-category-menu" location={"left"}/>
              </li>
              <li id="user-icon"onClick={()=>handleUserClick("sidebar")}><AiOutlineUser className="nav-icon"/></li>
            </ul>}
          </div>
