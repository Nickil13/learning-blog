        ...
        <div>
            <ul className="flex gap-5 w-full justify-center mt-5">
                {[...Array(pages).keys()].map((pageNum, index)=>{
                    return(
                        <li key={index} onClick={()=>handlePageClick(pageNum+1)} className={`${Number(page)===(pageNum+1) && "bg-purple-300"} px-2 py-1 rounded bg-gray-300 hover:bg-purple-600 cursor-pointer`}>
                            {pageNum+1}
                        </li>
                    );
                })}
