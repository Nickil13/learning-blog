    ...
    return (
        <div className="w-full">
            ...
            <div className="text-center mt-10">
                ...
                <Moment className="text-gray-400" format="MMM DD, YYYY" date={draft.createdAt}/>
                ...
                <div className="relative w-full h-0 pb-2/3 md:pb-9/16">
                    <img className="absolute inset-0 w-full h-full object-cover border-2 shadow-md dark:border-white" src={draft.image} alt={draft.title}/>
                </div>
                
                <div className="py-10 px-2 w-full">
                    <ReactMarkdown className="react-markdown">{draft.text}</ReactMarkdown>
                </div>

                {/* Edit button  */}
                {isLoggedIn && <button className="absolute right-0 -top-16 rounded-icon" onClick={()=>history.push(`/admin/edit-draft/${draft._id}`)}><FiEdit className="text-2xl"/>
                </button>}
            </article>}
