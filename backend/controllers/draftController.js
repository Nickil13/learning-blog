...
const getDrafts = asyncHandler(async (req,res)=>{
    ...
    const drafts = await Draft.find({}).limit(pageSize).skip(pageSize * (page-1));
...
const createDraft = asyncHandler(async (req, res) => {
    ...
    if(DraftTitleExists) return res.status(400).json({message: "Have a Draft with that title already!"});
